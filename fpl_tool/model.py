...
# ---------------------------
# V2 — Minutes + Poisson CS + log-scaled attacking (tuned)
# ---------------------------
def expected_points_v2(
    ...
    """
    V2 xPts (typical captain ~5–9; strong starters ~3–6):
      • p60 = min(start_prob*0.9*popularity_adj, minutes_share)
      • Appearance = 2 * start_prob  (baseline 2 for likely starters)
      • CS component requires 60' (multiplied by p60)
      • Attacking proxy uses log1p to avoid blowups, scaled by team xG and p60
      • Bonus proxy small & minutes-aware
    ...
    # ---- match context: team expected goals for & against (next GW) ----
    ctx = simple_expected_goals(fixtures, events)  # {team: {lam_for, lam_against, is_home, opp}}

    # ---- minutes share over finished GWs ----
    try:
    ...
    denom_mins = finished_gws * 90.0  # simple, DGWs ignored by design

    # ---- start probability & p60 gating ----
    ...
    # tiny popularity dampener (down-weights total punts)
    ...
    # ---- per-90 attacking proxy (log-scaled) ----
    ...
    # log scaling prevents tiny samples from exploding while keeping stars high
    atk_scaled = np.log1p(np.maximum(atk_raw, 0.0))

    # team xG + minutes (use p60 only; p60 already ≤ mins_ratio)
    df["atk_component"] = w_attack * df["lam_for"] * df["p60"] * atk_scaled

    # ---- appearance points: 2 for likely starters (expected) ----
    df["appear_pts"] = 2.0 * df["start_prob"]

    # ---- GK saves proxy (minutes-aware) ----
    ...
    df["xPts_v2"] = df["xPts_v2"].clip(lower=0.0, upper=18.0)
